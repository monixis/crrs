...
<script>
		...
		<div id="detailsType">
				...
				<div style="float:right; width:690px; height: 38px; text-align: center; font-size: 30px; color: #b31b1b;">
					...
					?>
          <script>
           // Need to check on initial page load in addition to value change
           $(function(){
             // Checks number of patrons
             var numPatrons = $("#numPatrons").val();

             // Modifies the form if there is only one patron
             if(numPatrons == 1){
               $("#check2").attr('hidden','true');
               $("#check2").removeAttr('required');
               $("#checkbox2").removeAttr('required');
               ...
               $("p#secEmailP").attr('hidden','true');
					...
					<FORM NAME="theForm" ID="theForm" ACTION="<?php echo base_url("?c=crr&m=reserveForm&resId=$resId")?>"  METHOD="POST">
                      ...
                      <?php } else if($timeAvailalbe==3){ ?>
						...
						<p class="resDet"><label class="label">Primary Marist Email:</label><input TYPE="email" NAME="primEmail" id="primEmail" value="<?php echo set_value('primEmail'); ?>" SIZE="40" class="ask_text_input" required/><img src="./icons/expand.png" class="viewNotes" id="viewNotes1" style="width:12px; height:12px; margin-left: 10px;"/><img src="./icons/addNotes.png" id="addNotes1" class="addNotes" style="width:14px; height:14px; margin-left: 7px;"/></br><div style="color:RED"><?php echo form_error('primEmail'); ?></div></p>
            ...
            <p class="resDet" id="secEmailP" <?php if ($pat_req == 1) { echo 'hidden'; } ?>><label class="label">Secondary Email:</label><input TYPE="email" NAME="secEmail" id="secEmail" value="<?php
            echo set_value('secEmail'); ?>" SIZE="40" class="ask_text_input" required/><img src="./icons/expand.png" class="viewNotes" id="viewNotes2" style="width:12px; height:12px; margin-left: 10px;"/><img src="./icons/addNotes.png" id="addNotes2" class="addNotes" style="width:14px; height:14px; margin-left: 7px;"/></br><div style="color:RED"><?php
